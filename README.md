...
В репозитории — полноценный fullstack: **Next.js 16** (фронт и API), **PostgreSQL** + **Prisma 7**, JWT-авторизация. Доски и задачи из БД подгружаются после входа; часть действий на канбане пока живёт только в **Redux** до перезагрузки страницы (см. раздел «Синхронизация данных»).
...
| Загрузка задач при открытии приложения | PostgreSQL → Redux |
| Очистка колонки | PostgreSQL + Redux |
| Создание / редактирование / удаление карточки, DnD | Только **Redux** (до перезагрузки) |

Порядок карточек в колонке на доске задаётся полем `order` в Redux. В API при загрузке порядок выводится из `dueDate` задачи в БД. Теги на карточках — UI-слой; в `Task` в схеме Prisma отдельного поля для тегов нет.
